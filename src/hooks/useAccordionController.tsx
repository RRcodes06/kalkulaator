...
function hasMeaningfulInputs(
  ...
  switch (sectionId) {
    ...
    case 'roles': {
      ...
      return (
        currHr.enabled !== initHr.enabled || currHr.payType !== initHr.payType || currHr.payAmount !== initHr.payAmount ||
        currMgr.enabled !== initMgr.enabled || currMgr.payType !== initMgr.payType || currMgr.payAmount !== initMgr.payAmount ||
        currTeam.enabled !== initTeam.enabled || currTeam.payType !== initTeam.payType || currTeam.payAmount !== initTeam.payAmount
      );
    }
    case 'strategy':
      return currentInputs.strategyPrep.hrHours !== initialInputs.strategyPrep.hrHours ||
    ...
    case 'ads':
      return currentInputs.adsBranding.hrHours !== initialInputs.adsBranding.hrHours ||
    ...
    case 'candidate':
      return currentInputs.candidateMgmt.hrHours !== initialInputs.candidateMgmt.hrHours ||
    ...
    case 'interviews':
      return currentInputs.interviews.hrHours !== initialInputs.interviews.hrHours ||
    ...
    case 'background':
      return currentInputs.backgroundOffer.hrHours !== initialInputs.backgroundOffer.hrHours ||
    ...
    case 'preboarding':
      return currentInputs.preboarding.devicesCost !== initialInputs.preboarding.devicesCost ||
    ...
    case 'onboarding':
      return currentInputs.onboarding.onboardingMonths !== initialInputs.onboarding.onboardingMonths ||
    ...
    case 'vacantImpact': {
      const curr = currentInputs.vacantPositionImpact;
      const init = initialInputs.vacantPositionImpact;
      return curr.mode !== init.mode ||
        curr.percentageUndone !== init.percentageUndone ||
        curr.monthlyPositionValue !== init.monthlyPositionValue ||
        curr.additionalHours !== init.additionalHours ||
        curr.avgHourlyCost !== init.avgHourlyCost ||
        curr.overtimeMultiplier !== init.overtimeMultiplier;
    }
    case 'risk':
      return true;
    default:
...
function hasBlockInputs(inputs: CalculatorInputs, sectionId: string): boolean {
  switch (sectionId) {
    case 'position':
      return inputs.positionTitle.trim() !== '' || inputs.hirePay.payType !== 'unset' || inputs.hirePay.payAmount > 0;
    case 'roles':
      const hr = inputs.roles.hr;
      const mgr = inputs.roles.manager;
      const team = inputs.roles.team;
      return (hr.enabled && (hr.payType !== 'unset' || hr.payAmount > 0)) ||
        (mgr.enabled && (mgr.payType !== 'unset' || mgr.payAmount > 0)) ||
        (team.enabled && (team.payType !== 'unset' || team.payAmount > 0));
    case 'strategy':
      return inputs.strategyPrep.hrHours > 0 || inputs.strategyPrep.managerHours > 0 || inputs.strategyPrep.teamHours > 0;
    case 'ads':
      return inputs.adsBranding.hrHours > 0 || inputs.adsBranding.managerHours > 0 || inputs.adsBranding.teamHours > 0 || inputs.adsBranding.directCosts > 0;
    case 'candidate':
      return inputs.candidateMgmt.hrHours > 0 || inputs.candidateMgmt.managerHours > 0 || inputs.candidateMgmt.teamHours > 0 || inputs.candidateMgmt.testsCost > 0;
    case 'interviews':
      return inputs.interviews.hrHours > 0 || inputs.interviews.managerHours > 0 || inputs.interviews.teamHours > 0 || inputs.interviews.directCosts > 0;
    case 'background':
      return inputs.backgroundOffer.hrHours > 0 || inputs.backgroundOffer.managerHours > 0 || inputs.backgroundOffer.teamHours > 0 || inputs.backgroundOffer.directCosts > 0;
    case 'other-services':
      return inputs.otherServices.length > 0;
    case 'preboarding':
      return inputs.preboarding.devicesCost > 0 || inputs.preboarding.itSetupHours > 0 || inputs.preboarding.prepHours > 0;
    case 'onboarding':
      return inputs.onboarding.onboardingMonths > 0 || inputs.onboarding.productivityPct > 0 || inputs.onboarding.extraCosts > 0;
    case 'vacantImpact': {
      const vpi = inputs.vacantPositionImpact;
      if (vpi.mode === 'uncovered') return vpi.percentageUndone > 0 || vpi.monthlyPositionValue > 0;
      return vpi.additionalHours > 0 || vpi.avgHourlyCost > 0;
    }
    case 'risk':
      return true;
    default:
...
export function AccordionControllerProvider({
  ...
  const isVisited = useCallback((sectionId: string): boolean => visitedSections.has(sectionId), [visitedSections]);

  const getSectionStateCallback = useCallback(
    (sectionId: string): SectionState => {
      return visitedSections.has(sectionId) ? 'completed' : 'not-started';
    },
    [visitedSections]
  );

  const hasNonZeroInputsCallback = useCallback(
    (sectionId: string): boolean => hasBlockInputs(inputs, sectionId),
    [inputs]
  );

  return (
    <AccordionControllerContext.Provider
      value={{ openSection, setOpenSection, getSectionState: getSectionStateCallback, hasNonZeroInputs: hasNonZeroInputsCallback, isVisited }}
    >
      {children}
    </AccordionControllerContext.Provider>
  );
...
export function useAccordionController() {
  ...
  if (!context) throw new Error('useAccordionController must be used within an AccordionControllerProvider');
  return context;
}
